...
# version 0.93 (2014/08/30)
# Added a toggle for modo's internal key reduction (may not always be a good fit)
# Added a toggle to apply Y rotation (0-90) from Faceshift to Z-scale in 0-1 range for each joint.
# author: Phil Stopford ( http://github.com/philstopford )
...
class BVHReader(bvh.BVHReader):
	...
	def __init__(self, filename):
		...
		self.keyReduction = False
		self.faceShift = False
		self.rootPin = False
		self.rootPinLimit = 0
...
# 		lx.out("ISROOT" ,node._is_root)
		...
		xfrmScl = lx.eval("query sceneservice item.xfrmScl ? %s" % node.id)
		'''		
 		if xfrmPos is None:
 		...
 		if xfrmScl is None:
 			lx.eval("transform.add type:scl")
 			xfrmScl = lx.eval("query sceneservice item.xfrmScl ? node.id")
		'''
		...
		for ch,val in zip(node.channels, nodeVal):
			if ch == "Xposition":
				...
				fsMode = False
				if (self.faceShift == True):
					myParentID = lx.eval('query sceneservice item.parent ?')
					if (lx.eval('query sceneservice item.name ? {%s}' % myParentID) == "Blendshapes"):
						# Map our 0-90 rotation range to 0-1 in the scale channel for Faceshift handling.
						fsMode = True
						lx.out("Blendshape - scale, not rotation")
						lx.eval("channel.key mode:add channel:{%s:%s} value:{%s}" % (xfrmScl,"scl.Z",val/90))
				if (fsMode == False): # If positive, we had faceshift blendshape data, so don't apply the rotation.
	...
	def keyReduce(self, itemList):
		if(self.keyReduction == False):
			return
		monitor.init(len(itemList))
		for item in itemList:
			xfrmPos = lx.eval("query sceneservice item.xfrmPos ? %s" % item)
			xfrmRot = lx.eval("query sceneservice item.xfrmRot ? %s" % item)
			xfrmScl = lx.eval("query sceneservice item.xfrmScl ? %s" % item)
			if (xfrmPos != None):
				lx.eval('select.channel {%s:%s} set' % (xfrmPos, "pos.X"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
				lx.eval('select.channel {%s:%s} set' % (xfrmPos, "pos.Y"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
				lx.eval('select.channel {%s:%s} set' % (xfrmPos, "pos.Z"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
			if (xfrmRot != None):
				lx.eval('select.channel {%s:%s} set' % (xfrmRot, "rot.X"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
				lx.eval('select.channel {%s:%s} set' % (xfrmRot, "rot.Y"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
				lx.eval('select.channel {%s:%s} set' % (xfrmRot, "rot.Z"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
			if (xfrmScl != None):
				lx.eval('select.channel {%s:%s} set' % (xfrmScl, "scl.X"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
				lx.eval('select.channel {%s:%s} set' % (xfrmScl, "scl.Y"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
				lx.eval('select.channel {%s:%s} set' % (xfrmScl, "scl.Z"))
				try:
					lx.eval('!channel.keyReduce')
				except:
					pass
...
if not lx.eval("query scriptsysservice userValue.isdefined ? BVHLoader.keyReduction"):
	lx.eval("user.defNew BVHLoader.keyReduction integer" )
	lx.eval("user.def BVHLoader.keyReduction list on;off")
	
if not lx.eval("query scriptsysservice userValue.isdefined ? BVHLoader.faceShift"):
	lx.eval("user.defNew BVHLoader.faceShift integer" )
	lx.eval("user.def BVHLoader.faceShift list on;off")
...
bvhRd.keyReduction = True if lx.eval("user.value BVHLoader.keyReduction ?") == "on" else False
bvhRd.faceShift = True if lx.eval("user.value BVHLoader.faceShift ?") == "on" else False
...
try:
	if (bvhRd.keyReduction == True):
		beforeImport = set(bvhRd.locatorList())
	bvhRd.read()
	if (bvhRd.keyReduction == True):
		afterImport = set(bvhRd.locatorList())
