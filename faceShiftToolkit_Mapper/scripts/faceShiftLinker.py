...
morphInfluenceNames = [] # morph deformer names belonging to our mesh item(s) in the scene.
...
def linkMorphs(meshID):
	...
	# This populates our morphInfluenceIDs and morphInfluenceNames lists
	...
	for index in range(len(morphInfluenceIDs)):
		...
		morphName = morphInfluenceNames[index]
...
def findMorphInfluences(meshID):
	...
	meshName = lx.eval('query sceneservice item.name ? {%s}' % meshID)
	lx.out('Mesh name is : {%s}' % meshName)
	numberOfMorphInfluences = lx.eval('query sceneservice morphDeform.N ?')
	lx.out('Morph influences : {%d}' % numberOfMorphInfluences)
	monitor = lx.Monitor( numberOfMorphInfluences )     
	for influence in range(numberOfMorphInfluences):
		...
		lx.eval('select.deformer {%s} set' % currentMorphID)
		deformerMesh = lx.eval('select.deformer mesh set')
		deformerMeshName = lx.eval('item.name ?')
		lx.out('Deformer mesh name is : {%s}' % deformerMeshName)
		if (meshName == deformerMeshName):
			lx.out('Matched influence {%s} to mesh {%s}' %(currentMorphName, deformerMeshName))
			morphInfluenceIDs.append(currentMorphID)
			morphInfluenceNames.append(currentMorphName)
		monitor.step()
